...
function TruckingOrder() {
  ...
  //State
  const [truckingCount, setTruckingCount] = useState(0)
  const [loader, setLoader] = useState(true)
  const { truckingId } = useParams()
  ...
  }, [user, truckingCount]) // eslint-disable-line react-hooks/exhaustive-deps

  //Functions
  const getLine = async () => {
    try {
      const response = await orderService.getTrucking(truckingId)
