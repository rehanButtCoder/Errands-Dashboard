...
function EditVehicleConfig() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await configurationService.update(data)
      debugger
  ...
  const getUsers = async () => {
    try {
      const response = await configurationService.getById(id)
      debugger
      if (response.data.code === 1) {
        setData(response.data.data)
        setResult(response.data.data.imagePath)
      }
...
export default EditVehicleConfig
